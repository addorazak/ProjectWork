...
{ 
  ...
  // a student class to hold all student attributes
  class student
  { 
    public:
    string name[10];
  ...
  // instantiating a student object k
  student k;
  // a staff class to hold all staff attributes
  class staff
  { 
    public:
    string FName[10];
  ...
  // instantiating a staff object t
  staff t;

  // menu runs until the loop counter variable exceeds 100 
 while(p<=100)
     {
      ...
      if(intro=="1")
 	      { 
	    while(stdMenu<=10)  //user has 10 times access to the menu
        {
          ...
          while (z<=10)
            {
                system("cls");
                i++;
                // inputing student attribute values
                ...
                cin>>k.ID[i-1];
                cout<<"Enter student "<<i<<"'s sex( M for male,F for female)\n";
                cin>>k.sex[i-1];
    ...
    // printing of academic record should user request for it
...
cout<<k.courses[r][i-1]<<"                                                    \t            "<<k.credit[r][i-1]<<"  \t   "<<k.gpa[r][i-1]<<"  \t   "<<k.gpt[r][i-1]<<endl;
    ...
    // code for editing of student record
